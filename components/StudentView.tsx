        ...
        scanIntervalRef.current = setInterval(() => {
          if (canvasRef.current && videoRef.current && videoRef.current.readyState === videoRef.current.HAVE_ENOUGH_DATA) {
            ...
            if (code) {
              ...
              // Extract canteen ID and details from URL
              try {
                ...
                if (canteenId) {
                  ...
                  // If canteen not found locally, try to reconstruct from QR params
                  if (!scannedCanteen) {
                    ...
                    if (name && campus) {
                      ...
                      // Save this new canteen to local device so it's remembered
                ...
                // If not a valid URL, try direct ID match (legacy support)
        ...
        {MENU_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative group p-4 rounded-2xl border-2 text-center transition-all duration-200 ${isSelected
            ...
            >
              ...
              <div className={`w-14 h-14 mx-auto mb-3 rounded-full flex items-center justify-center transition-colors ${isSelected ? 'bg-white text-primary-600 shadow-sm' : `${item.color} group-hover:scale-110 duration-300`
                }`}>
